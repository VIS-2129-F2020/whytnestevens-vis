...
```{r}
address_list = c("225 7th St SE, Washington, DC", 
...
```{r}
iso_10min_walk2 <- iso_10min_walk1 %>%
  mutate(transit_score = lengths(st_covers(geometry, metrostops2)))
```


```{r}
out2 <- data.frame(str_split_fixed(iso_10min_walk2$fromPlace, ",", 2))

out2 <- st_as_sf(x = out,                         
           coords = c("X2", "X1"),
           crs = 4326)
  
out2$transit_score <- iso_10min_walk2$transit_score
```
...
ggplot(dchoods) +
  ...
  theme(legend.position = c(.7,0),
        plot.title = element_text(hjust = 0.5))+
  labs(title = "Metro stops Acessibility Map")
```
